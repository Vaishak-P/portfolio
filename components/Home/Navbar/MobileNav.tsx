...
const MobileNav = ({showNav,closeNav}:Props) => {
  ...
  return (
    <div>
      ...
      ></div>
      {/* Navlinks  */}
      <div className={`text-white fixed ${navOpen} transform transition-all duration-500 delay-200 justify-center flex flex-col h-full w-[80%] sm:w-[60%] bg-[#0f0715] space-y-6 z-[2000]`}>
        {navLinks.map((navlink) => {
          return (
            <Link key={navlink.id} href={navlink.url}>
              <p className="nav__link text-[20px] ml-12 border-b-[1.5px] border-white sm:text[30px]">
                {navlink.label}
              </p>
            </Link>
          );
